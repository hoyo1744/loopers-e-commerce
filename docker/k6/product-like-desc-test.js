...
export default function () {
  const url = `${BASE_URL}${PATH}?brandId=${BRAND_ID}&sort=likes_desc&page=${PAGE}&size=${SIZE}`;
  const res = http.get(url, { tags: { endpoint: 'products_likes_desc' } });
  ...
  if (ok) {
    ...
    if (Array.isArray(products) && products.length > 0) {
      // like.count 내림차순 정렬 검증
      const counts = products.map((p) => p.like?.count ?? 0);
      const sorted = [...counts].sort((a, b) => b - a);

      check(null, {
        'like.count desc sorted': () => JSON.stringify(counts) === JSON.stringify(sorted),
      });
    }
  }
